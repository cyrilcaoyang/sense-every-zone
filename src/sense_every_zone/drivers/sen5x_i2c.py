...
Migrated from sen55-enviro-sensor — this is the proven raw driver that has
been running in production on a Pi Zero 2 W. It replaces the Sensirion SDK
approach with direct smbus2 transactions, avoiding the
sensirion-i2c-driver / sensirion-i2c-sen5x dependencies.
"""
...
class SEN5x:
    def __init__(self, bus_num: int = 1, addr: int = SEN5X_ADDR):
        self.addr = addr
    ...
    def set_auto_clean_interval(self, seconds: int):
        """Set the automatic fan-clean interval in seconds (0 = disabled)."""
        self._write_words(CMD_AUTO_CLEAN_IVL, [int(seconds) & 0xFFFF])

    def get_auto_clean_interval(self) -> int:
        """Read the current automatic fan-clean interval in seconds."""
        return self._read_words(CMD_AUTO_CLEAN_IVL, 1)[0]
